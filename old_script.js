...
content_button = $(`<button class="btn btn-default" style="${content_button_style}"><span class="glyphicon glyphicon-chevron-down"></span></button>`);
$('body').append(content_button);
content_style = 'position:fixed; bottom:35px; left:0; padding:10px; border:1px solid #CCC; border-radius:4px; background-color:rgba(200,200,200,0.9)';
...
function getGData(index) {
    if (index < D.g_list.length) {
        ...
        setTimeout(function () { getGData(index + 1); }, D.timespan);
    }
}
function getGirl() {
    g_list = [];
    $('#hh_game').contents().find('.girls_list').find('div[id_girl]').each(function (i, e) {
...
f_button.click(e => {
    ...
    $.ajax({
        type: 'POST',
        url: 'https://www.hentaiheroes.com/ajax.php',
        data: `class=Battle&action=fight&who[id_troll]=${f_select.val()}&who[id_world]${f_select.val() + 1}`,
    }).done(function (response) {
...
function upStat(count, stat) {
    ...
    $.ajax({
        ...
        c_feedback.val(`Stat ${D.c_list[stat - 1]} rised to ${response[`carac${stat}`]}${(count ? ` [${count} to go..]` : '')}`);
        setTimeout(function () { upStat(count - 1, stat); }, D.timespan);
    });
}
